...
window.addEventListener('scroll', function (ev) {
  ...
  if (scrollTop > lastScrollTop) {
    ...
    // const scrolledToBottom = (scrollTop + window.innerHeight) >= offsetHeight - 2;
    const scrolledToBottom = scrollTop + window.innerHeight >= offsetHeight;
    if (scrolledToBottom) {
...
export const scrollToTop = ({ratio = 20, smooth}) => {
  if (smooth) {
    window.scrollTo({
      top: 0,
      behavior: 'smooth',
...
export const scrollToBottom = ({ratio = 20, smooth}) => {
  const offsetHeight = getOffsetHeight();
  if (smooth) {
    window.scrollTo({
      top: offsetHeight,
    ...
    const currentScrollTop = getScrollTop();
    const scrollBottom = currentScrollTop + window.innerHeight - offsetHeight;
    if (scrollBottom < 0) {
